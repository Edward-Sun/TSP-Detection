        ...
        with ExitStack() as stack:
            ...
            for idx, batch in enumerate(data_loader):
                ...
                for per_image, output in zip(batch, outputs):
                    proposals = output._fields["proposal"]
                    all_attentions = output._fields["attention"]
                    ...
                    for head in range(48):
                        ...
                        if str(per_image["image_id"]) == "5193":
                        ...
                        elif str(per_image["image_id"]) == "139":
                            for i, edge_color in zip([7, 11, 12, 2, 1, 3], ["r", "g", "b", "c", "m", "y"]):
                                box_coord = (proposals[i][0], proposals[i][1], proposals[i][2], proposals[i][3])
                                visualizer.output.scale = 1.0
                                visualizer.draw_box(box_coord, edge_color=edge_color)
                        ...
                        elif str(per_image["image_id"]) == "632":
                            for i, edge_color in zip([0, 8, 4, 7, 5, 12],
                                                     ["r", "g", "b", "c", "m",
                                                      "y"]):
                                box_coord = (proposals[i][0], proposals[i][1],
                                             proposals[i][2], proposals[i][3])
                                visualizer.output.scale = 1.0
                                visualizer.draw_box(box_coord,
                                                    edge_color=edge_color)

                                visualizer.output.scale = 0.5
                                attention_boxes = torch.argsort(attentions[i],
                                                                descending=True)[
                                                  :5]
                                for j in range(5):
                                    box_id = attention_boxes[j]
                                    relevant_box = (
                                    proposals[box_id][0], proposals[box_id][1],
                                    proposals[box_id][2], proposals[box_id][3])
                                    visualizer.draw_box(relevant_box,
                                                        edge_color=edge_color,
                                                        line_style="--")

                        elif str(per_image["image_id"]) == "1000":
                            for i, edge_color in zip([1, 2, 0, 5, 8, 6],
                                                     ["r", "g", "b", "c", "m",
                                                      "y"]):
                                box_coord = (proposals[i][0], proposals[i][1],
                                             proposals[i][2], proposals[i][3])
                                ...
                                for j in range(5):
                                    ...
                                    visualizer.draw_box(relevant_box,
                                                        edge_color=edge_color,
                                                        line_style="--")
                        elif str(per_image["image_id"]) == "1268":
                            for i, edge_color in zip([1, 5, 8, 9, 4],
                                                     ["r", "g", "b", "c", "m",
                                                      "y"]):
                                box_coord = (proposals[i][0], proposals[i][1],
                                             proposals[i][2], proposals[i][3])
                                ...
                                for j in range(5):
                                    ...
                                    visualizer.draw_box(relevant_box,
                                                        edge_color=edge_color,
                                                        line_style="--")
                        elif str(per_image["image_id"]) == "2157":
                            for i, edge_color in zip([0, 1, 7, 17, 16, 2],
                                                     ["r", "g", "b", "c", "m",
                                                      "y"]):
                                box_coord = (proposals[i][0], proposals[i][1],
                                             proposals[i][2], proposals[i][3])
                            ...
                            for i in range(20):
                                box_coord = (proposals[i][0], proposals[i][1], proposals[i][2], proposals[i][3])
                                visualizer.draw_box(box_coord)
                                visualizer.draw_text("%d" % i, [_ + 5 for _ in box_coord[:2]])
